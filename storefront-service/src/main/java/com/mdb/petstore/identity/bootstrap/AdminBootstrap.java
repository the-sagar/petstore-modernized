...
public class AdminBootstrap implements ApplicationRunner {
    ...
    public AdminBootstrap(UserRepository users, PasswordEncoder encoder,
            @Value("${petstore.admin.bootstrap.username:}") String username,
            @Value("${petstore.admin.bootstrap.password:}") String password) {
        this.users = users;
    ...
    public void run(ApplicationArguments args) {
        ...
        if (users.existsByUsername(username)) {
            log.info("Admin bootstrap skipped existing username={}", username);
            return;
        }
        var now = Instant.now();
        var user = new User();
        ...
        try {
            ...
            log.info("Admin bootstrap skipped concurrently created username={}", username);
        }
    }
}
